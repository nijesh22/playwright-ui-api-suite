...
async def test_add_product_to_cart_logged_in_1(page):
    ...
    excepted_cart_title = "Men Tshirt"
    excepted_cart_price = "Rs. 400"

    assert await viewcart.get_product_tshirt() == excepted_cart_title, "Product title in cart does not match the expected value."
    print("✅ 'T shirt' product successfully added to cart and verified.")

    assert await viewcart.get_product_tshirt_price() == excepted_cart_price, "Product price in cart does not match the expected value."
    print("✅ 'T shirt Price'  product successfully added to cart and verified.")
