...
@pytest.mark.asyncio
async def test_attempt_to_access_profile_without_login_1(page,viewcart,productsdetails,brandproduct,products,home,signup,createaccount,loginhome):
    ...
    log.info("registered user cart verification Successful")
    ...
    for i in range(product_count):
        product_name = await viewcart.get_product_price_element_by_index(i)
        assert "Men Tshirt" in product_name or "Summer White Top" in product_name
    log.info("non registered user cart verification Successful")